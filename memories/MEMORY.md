...
- Battle persistence (debate.py `_finalize`): on win/loss/flee, snapshot transcript+verdicts+final_hp to durable `Encounter` columns (added via idempotent ALTER in `init_db`), write one embedded BATTLE `Memory` per party member (via WS-D `write_event`, content = POV summary + full transcript) so GEPA/RAG have real data, then `clear_conversation(eid)` evicts the heavy Redis keys (transcript+judge). `build_encounter_state` falls back to the durable snapshot when Redis is evicted. NOTE: GEPA still trains via synthetic self-play — wiring it to consume these stored BATTLE memories is the next step.
